...
(function() {
  ...
  _.last = function(array, n) {
	...
	return n === undefined ? array[array.length-1] : length ? array.slice(array.length-n) : array;
  };
  ...
  _.uniq = function(array) {
    var clean = [];
	/* FIRST ATTEMPT for(var i = 0; i<array.length; i++){
		if(clean.indexOf(array[i]) === -1){
			clean.push(array[i]);
			};
	*/
  ...
  _.map = function(collection, iterator){
	...
	/* WIP 
		var functionResults = function (value, key, collection) {
			newCollection.push(iterator(value, key, collection));
			}
			
		_.each(collection, functionResults)
	*/ 
	
	for (var i = 0; i<collection.length; i++){
		var newVal = iterator(collection[i]);
		newCollection.push(newVal);
		}
  ...
  _.some = function(collection, iterator) {
	...
	// Figure out a way to use _.every			
  };
